...
void ParameterEditorController::train()
{
    Fact* fact=getParam("XXXXX");
    fact->_containerRawValueChanged(_train);
}
void ParameterEditorController::changeValue(QString var, int value){
    if(var == "RC_OR_PID"){
        rc_or_pid=value;
    }
    if(var == "TRAIN"){
